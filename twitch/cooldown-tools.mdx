...
Note, the redeem will still be triggered but if its under cooldown nothing will happen and the user will just be refunded.
</Callout>
### Import Code
```md copy
...
Copy the `Import Code` and paste it into the `Import String`.
<Callout type="info" emoji="ℹ️">
This code will auto refund any action that is still in cooldown.
</Callout>
